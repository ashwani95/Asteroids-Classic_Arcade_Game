...
def group_collide(group,other_object):
    global lives,score,started,rock_group,missile_group
    for each in set(group):
        if each.collide(other_object):
            rock_group.remove(each)
            explosion_group.add(Sprite(each.pos, [0, 0], 0, 0, explosion_image,explosion_info, explosion_sound))
